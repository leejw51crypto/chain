...
fn do_cro_create_tx_deposit(network: u8, to_address_string: &str) -> Result<CroDepositTx> {
...
pub unsafe extern "C" fn cro_create_tx_deposit(
    ...
    match do_cro_create_tx_deposit(network, &to_address_string) {
        Ok(tx) => {
...
pub unsafe extern "C" fn cro_tx_add_txin_deposit(
    ...
    match do_cro_tx_add_txin_deposit(&mut tx, &txid_hex, txindex, &addr, coin) {
        Ok(_) => CroResult::success(),
        Err(_) => CroResult::fail(),
    }
}

fn do_cro_tx_sign_txin_deposit(
    address: &CroAddress,
...
pub unsafe extern "C" fn cro_tx_sign_txin_deposit(
    ...
    match do_cro_tx_sign_txin_deposit(&address, &mut user_tx, which_tx_in_user) {
        Ok(_) => CroResult::success(),
        Err(_) => CroResult::fail(),
    }
}
