...
use enclave_protocol::{
    ...
    QueryEncryptRequest, ENCRYPTION_REQUEST_SIZE,
};
...
impl<T: EnclaveProxy> TxValidationServer<T> {
    ...
    fn encrypt_data(&mut self, req: Box<QueryEncryptRequest>) -> EnclaveResponse {
        let tx_inputs = match req.tx_inputs {
            Some(inputs) => self.lookup_txids(inputs.iter().map(|x| x.id)),
            _ => None,
        };
        let result = match self.storage.get_last_app_state() {
            Some(state) => {
                let last_state =
                    ChainNodeState::decode(&mut state.as_slice()).expect("deserialize app state");
                let account = match req.op_sig {
                    Some(sig) => self.lookup_state(&req.txid, &sig, last_state.staking_version),
                    _ => None,
                };
                // TODO: fee in enclave?
                ...
                // this is done in tx-validation
                let response = self
                    .enclave
                    .process_request(IntraEnclaveRequest::Encrypt(Box::new(request)));
                match response {
    ...
    pub fn execute(&mut self) {
        ...
        loop {
            ...
            let result = self.socket_to_enclave.read(&mut request);
            if result.is_err() {
                continue;
            }
            let r_len = result.expect("get r_len");
            ...
            let resp = match mcmd {
                ...
                Ok(EnclaveRequest::GetTxInfo(req)) => {
                    EnclaveResponse::General("OK GetTxInfo".into())
                }
                Ok(EnclaveRequest::EncryptTxDirect(red)) => {
                    EnclaveResponse::General("OK EncryptTxDirect".into())
                }
                Err(e) => {
